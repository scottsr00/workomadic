...
/**
 * NYC Location Expansion Script
 * Finds and adds the best work locations in NYC based on Google reviews
 */
...
// Popular NYC neighborhoods for remote work
const NYC_NEIGHBORHOODS = [
  { name: 'Manhattan - SoHo', lat: 40.7235, lng: -74.0020 },
  { name: 'Manhattan - Chelsea', lat: 40.7484, lng: -74.0016 },
  { name: 'Brooklyn - Williamsburg', lat: 40.7182, lng: -73.9582 },
  { name: 'Manhattan - Flatiron', lat: 40.7411, lng: -73.9897 },
  { name: 'Manhattan - West Village', lat: 40.7358, lng: -74.0036 },
  { name: 'Manhattan - East Village', lat: 40.7265, lng: -73.9815 },
  ...
  // { name: 'Manhattan - Upper East Side', lat: 40.7736, lng: -73.9595 },
  // { name: 'Brooklyn - Fort Greene', lat: 40.6897, lng: -73.9749 },
  // { name: 'Queens - Long Island City', lat: 40.7505, lng: -73.9365 },
...
class NYCLocationExpander {
  ...
  async initialize() {
    console.log('🚀 Initializing NYC Location Expander...')
    ...
    console.log(`📊 Found ${existingLocations.length} existing NYC locations`)
  ...
  async analyzeReviews(place) {
    ...
    place.reviews.forEach(review => {
      if (review.text) {
        ...
        const workKeywords = ['work', 'laptop', 'wifi', 'quiet', 'study', 'coworking', 'remote', 'office']
  ...
  async addLocationToDatabase(place, neighborhood) {
    try {
      ...
      if (!nycCity) {
        console.error('❌ NYC city not found in database')
        return false
      }
      ...
      await prisma.photo.create({
        data: {
          ...
          alt: `${place.name} - NYC Work Location`,
          isPrimary: true,
  ...
  generateDescription(place, neighborhood, reviewAnalysis) {
    ...
    if (reviewAnalysis.workFriendlyScore > 0.3) {
      return `${baseDescription} Based on customer reviews, this location is particularly popular among remote workers and digital nomads.`
    }
  ...
  async expandLocations() {
    console.log('🔍 Starting NYC location expansion...')
...
async function main() {
  ...
  const expander = new NYCLocationExpander()
